    ...
    tools = [
        ...
        },
        {
            ...
            "description": (
                ...
                "Koristi 'boiler' za bojler (relej 1) ili 'socket' za uticnicu "
                "(relej 2). state=true ukljucuje, state=false iskljucuje."
            ),
            "inputSchema": {
                "type": "object",
                "properties": {
                    "device": {
                        "type": "string",
                        "enum": ["boiler", "socket"],
                        "description": "Uredjaj: 'boiler' (relej 1) ili 'socket' (relej 2)",
                    },
                    "state": {
                        "type": "boolean",
...
def tool_set_mode_arriving(arguments):
    ...
    # Procitaj trenutnu temperaturu za procjenu grijanja
    snap = state.snapshot()
    temp_info = ""
    if snap["temperature"] is not None:
        temp_info = f"\n  - Trenutna temperatura: {snap['temperature']}C"

    return (
        f"Mod dolaska aktiviran (dolazak za {minutes} minuta).\n"
        f"  - Bojler (relej 1): UKLJUCEN (voda ce biti topla za ~{min(30, minutes)} min){temp_info}\n"
        f"  - Uticnica (relej 2): UKLJUCENA\n"
...
def tool_get_device_status():
    ...
    lines.append(f"  Uticnica (relej 2): {snap['relay_socket']}")
...
def tool_toggle_relay(arguments):
    ...
    if device == "boiler":
        ...
        label = "Uticnica (relej 2)"
    else:
        return f"GRESKA: nepoznat uredjaj '{device}'. Koristi 'boiler' ili 'socket'."
